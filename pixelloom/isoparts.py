...
class Material:
    ...
    # Сила узора. Числа внутри фактур подобраны под шкалу из трёх-четырёх
    # ступеней и крупный предмет; на большой площади — стена дома, скат
    # кровли, поле — тот же разнотон переключает тон у каждого второго
    # пикселя, и материал читается рябью, а не фактурой. Коэффициент гасит
    # узор целиком, не трогая его рисунок: швы, ряды и валики остаются на
    # месте, пропадает только пиксельный шум между ними.
...
def surface_detail(part, p_local, normal, scale=1.0, kind=None):
    ...
    gain = getattr(part.material, "detail_gain", 1.0)
    if "+" in kind:
        total = 0.0
        for one in kind.split("+"):
            total = total + _detail_one(part, one.strip(), p_local, scale, off,
                                        normal)
        return total * gain
    return _detail_one(part, kind, p_local, scale, off, normal) * gain
...
def _detail_one(part, kind, p_local, scale=1.0, offset=(0.0, 0.0),
    ...
    if kind == "clapboard":
        ...
        board = (_hash01(np.floor(z / (0.42 * s)),
                         np.floor(u / (2.4 * s))) - 0.5) * 0.19
        # Продольная волокнистость внутри доски. На 0,04 доска шириной в шесть
        # пикселей оставалась ровной полосой: тень под кромкой есть, а внутри
        # пусто — стена читалась гофрой.
        grain = (_hash01(np.floor(z / (0.42 * s)),
                         np.floor(u / (0.16 * s)), 7.0) - 0.5) * 0.085
        # Тень под доской и блик на её верхней кромке усилены, разнотон самих
        # досок ослаблен: доски одной партии почти одного тона, а объём стене
        # даёт ступенька нахлёста.
        return wall(np.where(t < 0.16, -0.34,
                             np.where(t < 0.30, 0.20, board + grain)))
    if kind == "shingles":
        # Черепица и дранка: ряды со сдвигом и тенью под кромкой.
        #
        # У горизонтальной грани рисунок свой, по той же причине, что у
        # `pantile`: скат кровли собран уступами, и в изометрии видно верх
        # ступени, а не её торец.
        row_h = 0.34 * s
        row = np.floor(z / row_h)
        u = np.where(np.abs(x) > np.abs(y), y, x)
        ...
        rows = np.where(t < 0.20, -0.28, 0.05 + tone * 0.26)
        if flat_top is None:
            return rows
        size = np.array(part.size, dtype=float)
        along, across = (x, y) if size[0] >= size[1] else (y, x)
        row_t = np.floor(across / row_h)
        col_t = np.floor((along + (row_t % 2) * 0.35 * s) / (0.7 * s))
        t_t = (across / row_h) % 1.0
        seam = np.abs(((along + (row_t % 2) * 0.35 * s) / (0.7 * s) % 1.0)
                      - 0.5) > 0.44
        h_t = np.sin(row_t * 27.13 + col_t * 91.7) * 21374.13
        tone_t = (h_t - np.floor(h_t)) - 0.5
        # Редкий «другой» гонт: свежая дощечка светлее, старая темнее.
        # Без него между рядами остаются гладкие поля — на кровле мельницы
        # линтер мёртвых зон находил их пятнами в четверть ската.
        # Тон у каждого гонта свой: дранку тешут из разных чурбаков, и это
        # единственное, что ломает ровное поле между рядами. Редкий «другой»
        # гонт эту задачу не решает — между ним и соседом остаётся площадка
        # в четверть ската.
        body = 0.05 + tone_t * 0.34
        top = np.where(t_t < 0.24, -0.30, np.where(seam, -0.18, body))
        return np.where(flat_top, top, rows)
    if kind == "stucco":
    ...
    if kind == "stone":
        ...
        tone = (_hash01(row, col) - 0.5) * 0.09
        # Под верхней кромкой каждого ряда — тень: блок выступает из плоскости
        # стены, и на растре именно эта полоса делает кладку объёмной, а не
        # нарисованной сеткой.
        shade = (z / row_h % 1.0) < 0.16
        # Шов уходит на две ступени вниз, а не на одну. При −0,32 он попадал
        # в соседний тон, разница между ним и телом блока была едва заметна, и
        # тёсаная стена читалась ровной плитой: швов пятнадцать процентов
        # площади, а сетки не видно.
        return wall(np.where(seam_z | seam_u, -0.46,
                             np.where(shade, -0.15, tone)))
    if kind == "metal":
    ...
    if kind == "whitewash":
        ...
        u = x
        # Мазок чаще, чем был: период 3,1 на масштабе стены — это два метра,
        # то есть светлая полоса шириной с дверь. Восемь даёт мазок в ладонь.
        stroke = np.sin(z * 8.0 / s + np.sin(u * 4.2 / s) * 1.6)
        broad = np.sin(u * 0.62 / s + 1.1) * np.cos(z * 0.44 / s + 0.4)
        cell = 0.30 * s
        grit = _hash01(np.floor(u / cell), np.floor(z / cell)) - 0.5
        # Проступающая кладка: только там, где ряд и столбец совпали по
        # остатку. Прежде она включалась ещё и по широкой волне, а у той
        # период в метры — на стене дома проступало не «место, где слой
        # тоньше», а светлое облако размером с дверь.
        blot = _hash01(np.floor(u / (1.3 * s)), np.floor(z / (1.1 * s)))
        block = ((np.floor(z / (0.52 * s)) * 7
                  + np.floor(u / (0.9 * s))) % 7) == 0
        thin = block
        ...
        # Крап слабый, а широкий мазок и проступающая кладка — сильные: у
        # побелённой стены нет зерна, у неё есть неровность слоя. Мелкий крап
        # на площади в две сотни пикселей читается рябью, крупная волна —
        # рукой, которая клала известь.
        # Волна и пятна затирки держатся слабыми. На 0,10 и 0,05 они
        # переключали тон целыми областями, и на глухой стене выходил затёк
        # размером с человека — стена читалась залитой краской, а не
        # побелённой. Работает мазок кистью и проступающая кладка.
        # Широкая волна и пятна затирки убраны почти в ноль. Они работают
        # на предмете в ладонь и разваливаются на стене в два этажа: там их
        # период — метры, и стена читается разлитой краской. Остаются мазок
        # кисти и проступающая под тонким слоем кладка.
        return (stroke * 0.045 + broad * 0.010 + grit * 0.030
                - thin * 0.05)
    if kind == "rubble":
        ...
        # Шов глубокий, разнотон слабый. Наоборот — и кладка читается крупой:
        # камни спорят друг с другом тоном, а границы между ними не видно.
        # Работает именно шов, он и есть рисунок кладки.
        return np.where(mortar, -0.30, tone * 0.55 + chip * 0.12)
    if kind == "cobble":
        # Булыжник: округлые камни со своим тоном и светлой маковкой, между
        # ними тёмный шов.
        #
        # Вторая координата берётся по грани. На стене это высота, на полу —
        # глубина: у горизонтальной грани высота постоянна, и узор, считанный
        # по ней, вырождается в ровную заливку. Мощёный пол под аркадой рынка
        # именно так и вышел — серой плитой в четверть кадра.
        cell = 0.30 * s
        second = y if (normal is not None
                       and np.abs(np.asarray(normal)[..., 2]).mean() > 0.7) \
            else z
        gx = np.floor(x / cell)
        gz = np.floor(second / cell)
        ...
        dz = ((second / cell) % 1.0) - 0.5 + jz
        r = np.sqrt(dx * dx + dz * dz * 1.4)
        tone = (_hash01(gx + 5.0, gz + 9.0) - 0.5) * 0.13
        # Шов глубже маковки. На −0,14 он не дотягивал до шага ступени, и от
        # мостовой оставались только светлые чёрточки бликов: камни есть, а
        # границ между ними нет.
        return np.where(r > 0.40, -0.26, tone + np.clip(0.28 - r, 0, 1) * 0.24)
    if kind == "thatch":
        # Соломенная кровля. Снопы уложены рядами по скату и прижаты каждый
        # следующим, поэтому ряд читается не линией черепицы, а мягким
        # валиком: линия гуляет, под кромкой глубокая тень, выше неё —
        # освещённый горб. Внутри снопа видна штриховка стеблей, но она
        # обязана быть слабее разнотона самих снопов: на девяти пикселях в
        # метре стебель занимает полпикселя, и сильная штриховка читается
        # рябью, а не соломой.
        row_h = 0.78 * s
        u = np.where(np.abs(x) > np.abs(y), y, x)
        wob = np.sin(u * 1.6 / s) * 0.11 + np.sin(u * 0.7 / s + 1.4) * 0.07
        row = np.floor(z / row_h + wob)
        t = (z / row_h + wob) % 1.0
        # Штриховка стеблей и разнотон снопов ослаблены, а валик ряда усилен:
        # солома узнаётся волной уложенных рядов, а не зерном. Прежние числа
        # давали на скате рябь, за которой самой волны не было видно.
        stalk = (_hash01(np.floor(u / (0.075 * s)), row, 2.0) - 0.5) * 0.05
        bundle = (_hash01(np.floor(u / (0.52 * s)), row, 5.0) - 0.5) * 0.11
        rows = np.where(t < 0.13, -0.38,
                        np.where(t < 0.30, 0.19, stalk + bundle))
        if flat_top is None:
            return rows
        # Верхняя грань слоя — тот же настил, только сверху, и рядов на ней
        # столько же. Одного зерна мало: скат кровли собран уступами, у
        # верхних слоёв видна как раз горизонтальная грань, и без рядов они
        # выходят гладкими полями — на амбаре это половина кровли.
        size = np.array(part.size, dtype=float)
        along, across = (x, y) if size[0] >= size[1] else (y, x)
        wob_t = np.sin(along * 1.6 / s) * 0.11
        row_t = np.floor(across / row_h + wob_t)
        t_t = (across / row_h + wob_t) % 1.0
        stalk_t = (_hash01(np.floor(along / (0.075 * s)), row_t, 2.0)
                   - 0.5) * 0.05
        bundle_t = (_hash01(np.floor(along / (0.52 * s)), row_t, 5.0)
                    - 0.5) * 0.11
        crop = ((_hash01(np.floor(x / (0.16 * s)), np.floor(y / (0.16 * s)),
                         7.0) - 0.5) * 0.09)
        top = np.where(t_t < 0.15, -0.34,
                       np.where(t_t < 0.32, 0.17, stalk_t + bundle_t + crop))
        return np.where(flat_top, top, rows)
    if kind == "pantile":
        # Волновая черепица: ряды внахлёст, поперёк ряда — валик за валиком.
        # От плоской черепицы её отличает именно волна: гребень ловит свет,
        # ложбина уходит в тень, и кровля читается кровлей даже на скате
        # шириной в двадцать пикселей.
        #
        # Главное здесь — верхняя грань, а не стена. Скат в изометрии собран
        # уступами, и видно у них как раз ВЕРХ каждой ступени: погашенный
        # `wall` узор оставлял кровлю ровной заливкой в треть кадра. Поэтому
        # у горизонтальной грани свой рисунок: ряды идут поперёк конька, а
        # валики — вдоль стока. Направление берётся из формы самого тела:
        # у слоя кровли длинная ось и есть конёк.
        row_h = 0.30 * s
        wave = 0.26 * s
        u = np.where(np.abs(x) > np.abs(y), y, x)
        row = np.floor(z / row_h)
        t = (z / row_h) % 1.0
        ridge = np.cos((u / wave) * 2 * np.pi)
        tone = (_hash01(row, np.floor(u / wave), 3.0) - 0.5) * 0.07
        body = ridge * 0.22 + tone
        rows = np.where(t < 0.17, -0.32, body)
        if flat_top is None:
            return rows
        size = np.array(part.size, dtype=float)
        along, across = (x, y) if size[0] >= size[1] else (y, x)
        ridge_t = np.cos((along / wave) * 2 * np.pi)
        t_t = (across / row_h) % 1.0
        tone_t = (_hash01(np.floor(across / row_h), np.floor(along / wave),
                          3.0) - 0.5) * 0.07
        # Тень под кромкой ряда держится глубокой: на верхней грани уступа
        # ряд занимает два-три пикселя, и всё, что слабее трети ступени
        # палитры, вырождается в рябь.
        top = np.where(t_t < 0.26, -0.34, ridge_t * 0.24 + tone_t)
        return np.where(flat_top, top, rows)
    if kind == "logs":
        # Венцы сруба: бревно круглое, поэтому тон внутри венца меняется
        # плавно от верхней кромки к нижней, а между венцами лежит паз —
        # узкая глубокая тень. Ровная синусоида без паза читается гофрой.
        row_h = 0.34 * s
        t = (z / row_h) % 1.0
        row = np.floor(z / row_h)
        u = np.where(np.abs(x) > np.abs(y), y, x)
        round_ = np.cos((t - 0.55) * 2.6) * 0.5 + 0.5
        knot = _hash01(np.floor(u / (0.9 * s)), row, 6.0) > 0.90
        grain = (_hash01(np.floor(u / (0.20 * s)), row, 8.0) - 0.5) * 0.09
        body = (round_ - 0.5) * 0.34 + grain * 0.5
        return wall(np.where(t < 0.10, -0.34,
                             np.where(knot, body - 0.16, body)))
    if kind == "chipped":
    ...
    if kind == "bark":
        # Кора старого дерева. Держится на бороздах, идущих вдоль ствола, а
        # не на зерне: зерно на девяти пикселях в метре даёт грязь, борозда
        # даёт тень в пиксель и читается на любом расстоянии.
        #
        # Линия борозды гуляет двумя синусами разной длины. Ровные вертикали
        # превращают ствол в гофрированный лист: у дерева ни одна борозда не
        # идёт параллельно соседней дольше полуметра.
        pitch = (np.pi / 9.0) * s
        # Гуляние борозды слабое: при 0,30 периода борозды уходили по
        # диагонали, и ствол читался связкой брёвен, а не корой. Борозда
        # обязана идти ВДОЛЬ ствола, а виться — на десятую долю шага.
        wob = np.sin(z * 1.30 / s) * 0.11 + np.sin(z * 0.51 / s + 2.1) * 0.07
        u = ang / pitch + wob
        t = u % 1.0
        plate = (_hash01(np.floor(u), np.floor(z / (0.85 * s)), 11.0)
                 - 0.5) * 0.11
        grain = (_hash01(np.floor(u * 3.0), np.floor(z / (0.17 * s)), 4.0)
                 - 0.5) * 0.045
        return wall(np.where(t < 0.15, -0.34,
                             np.where(t < 0.27, 0.13, plate + grain)))
    if kind == "gills":
        # Пластинки под шляпкой гриба. В изометрии низ шляпки камере не
        # виден, зато виден её торец — и на нём пластины читаются частой
        # вертикальной гребёнкой. Период мельче, чем у любой другой фактуры
        # пака: пластинки у гриба идут гуще, чем доски у стены.
        pitch = (np.pi / 24.0) * s
        t = (ang / pitch) % 1.0
        tone = (_hash01(np.floor(ang / pitch), np.floor(z / (0.6 * s)), 19.0)
                - 0.5) * 0.07
        return wall(np.where(t < 0.44, -0.27, 0.15 + tone))
    if kind == "cap":
        # Шляпка гриба сверху: кольцевые валики от середины и редкие светлые
        # бородавки. Валик обязателен — гладкая шляпка в изометрии читается
        # надувным мячом, а не грибом, и на ней негде поймать свет.
        r = np.sqrt(x * x + y * y)
        ring_h = 0.55 * s
        t = (r / ring_h) % 1.0
        wart = _hash01(np.floor(x / (0.38 * s)), np.floor(y / (0.38 * s)),
                       13.0)
        spot = np.where(wart > 0.87, 0.26, 0.0)
        tone = (_hash01(np.floor(r / ring_h),
                        np.floor(ang / ((np.pi / 6.0) * s)), 23.0)
                - 0.5) * 0.08
        top = np.where(t < 0.24, -0.22, 0.07 + tone) + spot
        if flat_top is None:
            return top
        # На торце слоя те же кольца, только видно их сбоку: полоса ряда.
        side = np.where(((z / (ring_h * 0.7)) % 1.0) < 0.26, -0.20, 0.06)
        return np.where(flat_top, top, side)
    if kind == "facet":
        # Грани кристалла. Здесь всё наоборот против камня: шума нет вовсе,
        # работают крупные плоские сектора и резкое ребро между ними. Шум на
        # кристалле убивает его главную примету — то, что он гладкий.
        pitch = (np.pi / 3.2) * s
        u = ang / pitch
        edge = np.abs((u % 1.0) - 0.5) > 0.44
        tone = (_hash01(np.floor(u), np.floor(z / (1.7 * s)), 17.0)
                - 0.5) * 0.34
        return np.where(edge, -0.24, tone)
    if kind == "fade":
